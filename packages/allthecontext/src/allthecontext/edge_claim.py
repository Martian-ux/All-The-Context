...
class EdgeClaimBundle:
    ...
    def decode(cls, value: str) -> EdgeClaimBundle:
        ...
        if any(
            ...
            raise EdgeClaimError("Edge claim field types are invalid")
        expires_at = parsed["expires_at"]
        if isinstance(expires_at, bool) or not isinstance(expires_at, int) or expires_at < 1:
            raise EdgeClaimError("Edge claim expiry is invalid")
        bundle = cls(**parsed)
        if not 32 <= len(bundle.claim_id) <= 200 or not 1 <= len(bundle.vault_id) <= 200:
        ...
        try:
            bytes.fromhex(bundle.owner_secret_hash)
        except ValueError as exc:
            raise EdgeClaimError("Edge owner hash is invalid") from exc
...
class EdgeClaimPrivate:
    ...
    def __post_init__(self) -> None:
        _decode(self.signing_private_key, expected=32)
        _decode(self.encryption_private_key, expected=32)
...
def decrypt_claim(
    ...
    try:
        if set(envelope) != {"ephemeral_public_key", "nonce", "ciphertext"} or any(
            not isinstance(value, str) for value in envelope.values()
        ):
            raise EdgeClaimError("claim credential envelope is invalid")
        ...
        raise EdgeClaimError("claim credential envelope is invalid") from exc
    if not isinstance(parsed, dict) or set(parsed) != {
        "replication_secret",
        "replication_token",
    }:
        raise EdgeClaimError("claim credentials are invalid")
    secret = parsed["replication_secret"]
    token = parsed["replication_token"]
    if (
        ...
        raise EdgeClaimError("claim credentials are invalid")
    return {"replication_secret": secret, "replication_token": token}
...
def decrypt_forward_request(private_key: str, envelope_json: str, associated_data: bytes) -> bytes:
    ...
    try:
        ...
        if not isinstance(envelope, dict) or set(envelope) != {
            ...
            raise EdgeClaimError("forwarding envelope is invalid")
...
class EdgeClaimStore:
    ...
    def challenge(self) -> str:
        ...
        with self._lock, self._connection() as connection:
            ...
            connection.execute(
                "DELETE FROM edge_claim_challenges WHERE challenge_hash IN "
                "(SELECT challenge_hash FROM edge_claim_challenges "
                "ORDER BY expires_at ASC LIMIT MAX(0, "
                "(SELECT COUNT(*) FROM edge_claim_challenges)-?))",
