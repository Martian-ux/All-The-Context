...
class EdgeConnectionStore:
    ...
    def connect(
        ...
        *,
        ...
        """Claim and pair one Edge while serializing credential/state transitions."""
        ...
        if material.claim_bundle is not None and material.claim_private is not None:
            ...
            try:
                ...
                try:
                    ...
                    # Core may have crashed after Edge acknowledged the claim but
                    # before the local private claim material was removed.
                    resumed_after_ack = True

                if not resumed_after_ack:
                    ...
                    credentials = decrypt_claim(
                        material.claim_bundle, material.claim_private, envelope
                    )
...
class EdgeSyncManager:
    ...
    def _execute_forward_request(self, envelope: dict[str, Any]) -> dict[str, Any]:
        ...
        except (EdgeClaimError, ValueError, json.JSONDecodeError):
            return {"state": "unavailable"}
            ...
            return self._bounded_forward_response(
                {
                    "state": "available",
                    "found": True,
                    "item": self._forward_record(record.model_dump(mode="json")),
                }
            )
    ...
    def _forward_record(record: dict[str, Any]) -> dict[str, Any]:
        """Project a Core record into the same public schema served by Edge."""
