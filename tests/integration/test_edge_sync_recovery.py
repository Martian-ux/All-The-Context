...
def test_claim_ack_crash_resumes_without_redeploying_edge(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("PYTHON_KEYRING_BACKEND", "keyring.backends.null.Keyring")
    core = CoreService(CoreConfig.in_directory(tmp_path / "core"))
    ...
    original_replace = connections.replace_bundle
    crashed = False

    def crash_after_ack(material, bundle, *, preserve_claim=True):  # type: ignore[no-untyped-def]
        nonlocal crashed
        if not preserve_claim and not crashed:
            crashed = True
            raise RuntimeError("simulated crash after Edge acknowledgement")
        return original_replace(material, bundle, preserve_claim=preserve_claim)

    try:
        with TestClient(app, base_url=PUBLIC_URL) as client:
            monkeypatch.setattr(connections, "replace_bundle", crash_after_ack)
            with pytest.raises(RuntimeError, match="claim could not be completed"):
                connections.connect(PUBLIC_URL, client=client)
            interrupted = connections.material()
            assert interrupted is not None
            assert interrupted.claim_bundle is not None
            assert claim_store.acknowledged()

            monkeypatch.setattr(connections, "replace_bundle", original_replace)
            resumed = connections.connect(PUBLIC_URL, client=client)
            assert resumed.edge_url == PUBLIC_URL
            recovered = connections.material()
            assert recovered is not None
            assert recovered.claim_bundle is None
    finally:
        oauth_store.close()
        service.close()
