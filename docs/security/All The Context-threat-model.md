...
| TM-020 | Release-service/build attacker or interrupted installer | Configured update check or cutover | Substitute metadata/artifact, exploit archive paths, truncate bytes, or leave a half-applied version | Code execution, vault corruption, availability loss | Application, vault, update trust | Exact signed manifest, active/channel-scoped Ed25519 key, HTTPS/no redirect/time/size bounds, immutable version URL, streamed signed size/hash, safe cross-platform ZIP paths; Windows-only strict path-bound journal, independent RunOnce helper, cross-process lock, stopped-Core final backup, startup race guard, exact app/MCP/updater diagnostics, real loopback health, idempotent resume, and full binary/database rollback (`release_manifest.py`, `updater.py`, `windows_update_helper.py`) | Production public key/channel are absent; native artifacts intentionally lack paid publisher identity; frozen Windows evidence is same-version; macOS/Linux automatic rollback is absent | Complete offline key ceremony, explicit unsigned-build disclosure, real Ed25519-signed N-1 Windows drill, and observed macOS/Linux rollback before enabling those adapters | Signature/checksum failures, fixed journal error code, persisted recovery phase | low | critical | high |
