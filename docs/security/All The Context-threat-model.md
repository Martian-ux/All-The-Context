...
| TM-013 | Assistant/tool text or archive instructions are mistaken for user memory | False canonical context after careless review | Provider role normalization; only user messages and dedicated provider memory/profile fields create pending candidates; no imported item bypasses approval | A provider can mislabel roles or synthesize inaccurate memory; provenance and review remain required |
| TM-014 | Provider schema drift silently omits conversations | Incomplete portable memory | Preserve byte-exact raw source; report recognized files/messages, warnings, unavailable material, and limitations; version parser sessions for reprocessing | Provider schemas are not contractual, especially Grok |
