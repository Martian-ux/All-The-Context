...
| Update interruption or unhealthy replacement damages the vault | Persist transaction phases, take and integrity-check a SQLite backup before handoff, make recovery idempotent, require replacement version plus loopback health, and retain recovery material when automatic binary rollback is unavailable |
| A platform archive is mistaken for a safe self-updater | Keep every real adapter manual-required, including Windows, until native signing and an independent journaled cutover that restores both binary and database pass packaged crash/lock/health drills |
