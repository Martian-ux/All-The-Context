...
| A platform archive is mistaken for a safe production self-updater | Windows packaged crash/resume and failed-health rollback drills now pass, but the artifact is unsigned and the frozen drill is same-version; require the offline key ceremony, immutable public channel, Authenticode/publisher gates, and a real signed N-1 drill before production claims. Keep macOS/Linux manual-required. |
