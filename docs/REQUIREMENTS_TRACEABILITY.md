...
| Optional administration UI, no memory inbox | `apps/dashboard`; Review route/forms removed, Context default, Activity/provenance, context and source delete/undo, version restore | Local evidence: 26 tests and type check pass; production rebuild, dependency audit, and browser smoke pending for this worktree |
| Approval-free reproducible demo | `scripts/demo.py`, `tests/e2e/test_demo.py`; automatic finish-to-retrieve, restart, correction/delete, revocation, encrypted restore | Included in the passing 509-test Windows Python 3.14 suite; hosted Python 3.12 matrix pending |
