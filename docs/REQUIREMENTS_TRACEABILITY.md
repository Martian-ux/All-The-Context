...
| Windows diagnostic custody | `.github/workflows/ci.yml`; `scripts/probe_packaged_process_inventory.py` | Implemented at source level. Windows desktop smoke writes to a dedicated directory and an always-run pinned upload action retains only JSON observations under `packaged-first-run-process-inventory-windows`; missing files are an upload error. The local probe reuses the same PowerShell/CIM implementation. Hosted, artifact, and release acceptance remain separate. |
