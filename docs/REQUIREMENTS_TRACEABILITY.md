...
| Claude Code explicit-user memory Core contract | `core/app.py`, `ingestion.py`, `models.py`, `security.py`, `storage.py`, `tests/unit/test_claude_code_memory.py`, `tests/integration/test_claude_code_memory_core_api.py`, `docs/protocols/CLAUDE_CODE_MEMORY.md`, ADR-162 | Implemented locally: dedicated remember/correct/reversible-forget routes require a separate principal with exactly `context:propose` plus `witness:explicit_user_statement`; the existing Claude Code read principal remains exactly `context:read`. Core rejects authority fields, assigns origin/sensitivity/availability/ACL/disposition, applies bounded UUIDv4 idempotency, reuses candidate/correction/tombstone machinery, refuses direct secret-like payloads, and has no Relay fallback. Ordinary prompt/model/tool/provider/imported text and MCP elicitation are not direct-user evidence. This is Core/API evidence only; client hook/config/setup, live/private acceptance, release, and macOS claims remain out of scope |
...
| Opt-in Claude Code explicit memory command client/config/setup boundary | `claude_code_hook.py`, `claude_code_config.py`, `http_client.py`, `mcp_adapter.py`, `desktop_setup.py`, `wizard.py`, packaged headless setup in `desktop.py`, focused unit tests, `docs/protocols/CLAUDE_CODE_HOOK.md`, ADR-162 | Implemented only for exact user-typed `/atc-remember`, `/atc-correct`, and `/atc-forget` through the official `UserPromptExpansion` `command_name`/`command_args` contract. The feature is off by default, preserves the read principal, provisions a separate exact two-grant write principal, keeps bounded opaque pending state with a content commitment, requires native exact-payload `confirm=true` before Core mutation, uses the pending command ID only as `idempotency_key` in strict route payloads, retries one ambiguous transport failure with the identical payload/key, and reports unresolved outcomes as unknown. It preserves unrelated Claude settings/commands transactionally, calls only the narrow Core Claude Code memory routes with no Relay fallback, and blocks on missing/revoked Core. Ordinary UserPromptSubmit remains read-only; no Core ingestion/policy/API files, transcript/session/cwd/attachment capture, live/private acceptance, macOS work, or product/release exit is claimed. |
