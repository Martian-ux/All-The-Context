...
| Claude Code UserPromptSubmit pre-generation runtime slice | `claude_code_hook.py`, profile selection in `mcp_adapter.py`, Core-only bootstrap client path, `tests/unit/test_claude_code_hook.py`, `tests/integration/test_claude_code_hook_core_api.py`, `docs/protocols/CLAUDE_CODE_HOOK.md`, ADR-159 | Implemented as an isolated pre-generation-only hook profile: strict bounded `prompt`/`cwd`/`session_id` schema, actual SDK one-text-item JSON serialization, authorized-content-only 8,000-character output, content-free failure/auth/timeout paths, loopback Core-only/no-Relay boundary, and read-only/revocation coverage. This is not supported configured Claude Code client setup, L1 lifecycle support, a product exit, a release claim, or live provider/client acceptance; ordinary MCP remains L0 |
