...
| WIN-AV-02 — installed Windows executables are individually identifiable and reassessed | Future installed-component inventory covering main, MCP, recovery, and updater executables; reproducible per-component provenance; exact-component Microsoft reassessment; optional Authenticode verification when free, donated, or sponsored | Not implemented. Paid signing is not a community-release requirement under ADR-028/ADR-166. Component provenance and reassessment remain required follow-up outside the Continuous Capture product PR |
