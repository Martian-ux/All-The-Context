...
|---|---|---|
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075/ADR-086/ADR-088; `STATUS.md`; this matrix | Active plan with five dependency phases, 20 exact prepublication receipt gates, postpublication `BETA-R05`/`BETA-O01`, mapped work packages, non-waivable failures, and a post-V1 horizon. ADR-086 fixed the public-beta floor to Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64. The live unpublished `0.1.0-beta.1` four-platform identity is draft `367337056` / source `563a397` / RC `31285545048` / candidate `ba17eeec…42842ebe` (55 assets) and is never retargeted, deleted, or published; the earlier `360008392` / `488150…` episode is historical only. Exact main `6be7e1d` hosted CI exposed a nondeterministic Windows provider-acceptance failure; inspection identified the missing explicit Core-close boundary before owned-vault removal. Candidate dispatch stays blocked pending the ADR-088 merge and exact-main green. High-level A-01 through A-11 scope is otherwise fixed |
