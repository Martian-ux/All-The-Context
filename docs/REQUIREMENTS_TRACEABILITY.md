...
|---|---|---|
| Frozen Python dependency audit | `pyproject.toml`; `uv.lock`; `scripts/dependency_audit.py`; ADR-091 | The dev-and-packaging export fails closed on newly published advisories. The 2026-08-21 response to PYSEC-2026-3721 / CVE-2026-13346 requires fixed `pip>=26.2,<27` while keeping pip outside runtime dependencies. A fresh hosted matrix is required before merge, and this source repair grants no beta acceptance credit |
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075/ADR-086/ADR-088/ADR-089; `STATUS.md`; this matrix | Active plan with five dependency phases, 20 exact prepublication receipt gates, postpublication `BETA-R05`/`BETA-O01`, mapped work packages, non-waivable failures, and a post-V1 horizon. ADR-086 fixed the public-beta floor to Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64. The live unpublished `0.1.0-beta.1` four-platform identity is draft `367337056` / source `563a397` / RC `31285545048` / candidate `ba17eeec…42842ebe` (55 assets) and is never retargeted, deleted, or published; the earlier `360008392` / `488150…` episode is historical only. The unpublished `0.1.0-beta.2` Windows/Linux-only identity remains an occupied historical draft and is not rebound. ADR-089 records the source `:focus-within` correction that advances the active source version to `0.1.0-beta.3`; the resulting live unpublished candidate is draft `371617909` / source `89f3973` / RC `32010253144` / candidate `804afcd9…1d77f369` (31 assets). It remains unsigned and unpublished, and its exact-candidate acceptance and human release gates remain open. High-level A-01 through A-11 scope is otherwise fixed |
| Post-V1 zero-routine-friction platform direction | ADR-090; `product/ZERO_FRICTION_PLATFORM.md`; `product/ZERO_FRICTION_EXECUTION_PLAN.md`; `product/PRODUCT_REQUIREMENTS.md` | Accepted product and execution direction only. Core remains authoritative; connected text remains inert untrusted data; authorization and lifecycle resolution precede derived work; capability levels prevent L0 integrations from claiming lifecycle hooks; correction, deletion, retention, and purge dependencies close future ATC influence. Phase 0 preserves the beta.3 boundary. `ZF-*` implementation issues begin only after tracker reconciliation and beta publication, and no graph, connector, working-state, learned-retrieval, remote, or stable-SDK claim is credited to beta.3 |
| Cross-platform Core | `config.py`, `lifecycle.py`, `platform_compat.py`; platform/package smoke tests; retained `macos_acceptance_preflight.py`; ADR-086 | Public-beta floor: Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64 GNOME with working Secret Service/GNOME Keyring; other Linux environments are experimental. macOS code and hosted regressions remain for source portability, but macOS is unsupported and excluded from candidate assets, claims, and acceptance receipts. Exact supported-target clean-machine and final release-SHA receipts remain pending |
...
| Cross-platform source CI | `.github/workflows/ci.yml` source, dashboard, native-package matrices, and retained content-free Mac preflight | The hosted matrix remains a source-health regression across Windows/macOS/Linux. Under ADR-086, only Windows and Linux are product support targets; hosted Mac jobs grant no support or receipt credit and are absent from release-candidate composition. Final release-SHA source health and supported clean-machine receipts remain pending |
| Supported desktop packaging | Windows installer and Linux portable archive; version-matched recovery helper/mode; bounded native-tool failure diagnostics and a single 30-second managed-Core startup window; packaged first-run smoke startup-key cleanup (`remove_smoke_windows_startup_key`); retained Mac packaging code under ADR-086 | Windows and Linux packaging/recovery surfaces are integrated. The official candidate matrix, inventory, release notes, and publication verifier accept only Windows x86-64 and Linux x86-64. Mac app/DMG code and CI regressions remain in source but create no consumer asset or support evidence. Exact supported release artifacts and real-machine/downloaded-artifact receipts remain pending |
| Signed community updates | Ed25519 manifests/keyring with active beta key `release-2026-a`, installed/frozen package identity, trust-backed available-channel diagnostics, explicit `unpublished` state for only the empty canonical beta channel, owner-admin immutability preflight without an Actions admin token, authenticated paginated draft collision detection and numeric release-ID controls through publication (ADR-068), canonical packaged beta endpoint, pinned GitHub release-asset redirect, checksums, SBOM/provenance, Windows recovery helper, bounded non-sensitive rollback failure state, and one exact-state persisted rollback-smoke re-entry (ADR-082); `accept_exact_candidate` reopens a verified same-version offer for transactional acceptance smoke without network I/O; `publication_gate.py` + receipt recompute (ADR-059) refuse forged/incomplete/mixed digests | Source machinery for same-version beta exact-candidate OTA acceptance/rollback is honest and fail-closed. Unpublished immutable-mode drafts are now uniquely bound by numeric ID, target, state, and exact asset names/digests/sizes; any matching draft closes the version slot even while tag routes return 404. The Windows packaged smoke may re-enter only `rolling_back` / `rollback_retry_required` once, then must still prove terminal rollback, restored binaries/database, prior-Core health, uninstall, and cleanup; every other state or repeated failure blocks. The live unpublished four-platform `beta.1` draft is ID `367337056` / source `563a397` / RC `31285545048` / candidate `ba17eeec…42842ebe` (55 assets) and remains unpublished; the earlier `360008392` / `488150…` episode is historical only. That occupied version cannot be reused. The unpublished `beta.2` identity is likewise occupied and is not rebound. A fresh Windows/Linux-only `beta.3` candidate, key-backup restore tests, deliberate sole-maintainer approval, offline-signed Windows channel bytes, public download smoke (BETA-R05), and exact packaged candidate rollback receipt remain pending; a real first-published-beta-to-successor N-1 transaction remains post-V1 |
