...
| Start one interruptible non-daemon thread after Core is ready and stop it on close | FastAPI lifespan; `CoreService.close`; bounded admin `stop`/`disable`; unbounded `shutdown` | Implemented locally: start is after Core ready when gates pass, and admin enable can start or revive the worker; disable/stop join with a bound; lifespan `finally` and `CoreService.close` wait until the non-daemon worker is dead before store/instance-lock release; the worker is never daemonized and is not cancelled; overlapping cycles are refused by an in-process global cycle lock, distinct from the coordinator's cross-process per-source lease; sidecar write plus start/stop are serialized by a control mutex and joins do not hold that mutex; disable-then-enable during an in-flight cycle eventually runs while gates stay enabled; `max_workers` is 1 |
