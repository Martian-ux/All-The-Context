...
| Requirement | Implementation/evidence | Status |
|---|---|---|
| UPDATER-01 — a verified staged candidate and detached recovery path must not silently change before use | `updater.py` install-time manifest/artifact identity and checksum checks; `PlatformInstaller` opened-archive recheck; version-2 `UpdateJournal` recovery-helper binding; `windows_update_helper.py` pre-launch/pre-subprocess verification; `tests/unit/test_updater.py` and `tests/unit/test_windows_update_helper.py` adversarial swap, missing-binding, reparse/hardlink, and replacement checks | Implemented locally as source-level hardening. The persisted signed manifest, staged ZIP, copied recovery helper, replacement, installed application, and rollback sources fail closed when their plain-file identity or SHA-256/size binding is absent or changes. Focused tests pass locally. Prepared-journal publication crash recovery, journal replacement/generation binding, recursive cleanup races, database-backup identity, the final portable validation-to-process-creation interval, exact packaged/live behavior, and release acceptance remain open |
