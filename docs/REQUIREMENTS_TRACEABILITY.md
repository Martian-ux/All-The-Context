...
| Wave 3 falsification program | ADR-046; active Wave 3 manifest and governance test; five visible B01/O01/P01/E01b/M2 falsification tasks from one immutable experiment base plus one later metadata-only external-artifact intake from the governance-only coordinator head | Active with coordinator-only integration, new-file-only ownership, no external code/models/providers or operator Core, no raw adversarial payload access, and explicit preservation of failed/unsupported/held/killed/not-exercised cells; no result accepted yet |
