...
| ZF-013 — project graph in Memory Lab | `bench/zf013_project_graph_contract.json`, `bench/zf013_project_graph_fixtures.json`, `bench/zf013_project_graph_benchmark.py`, `tests/unit/test_zf013_project_graph_benchmark.py`, `docs/research/ZF013_PROJECT_GRAPH_EVALUATION.md` | Lane A evaluation harness only: frozen sanitized comparison of current Retrieval V3, structured project filters, deterministic capsules, lexical typed one-hop, bounded two-hop, and six relation-family ablations. Local run passes the exact recall, Project CAOS, wrong-project, stale, deleted, purged, unnecessary-disclosure, receipt-determinism, cycle, fan-out, latency, and ablation-decision gates. No graph store, graph runtime, learned relation, production expansion, or graph promotion is implemented or credited; ZF-013 remains open |
