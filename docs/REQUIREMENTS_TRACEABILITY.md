...
|---|---|---|
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075/ADR-086; `STATUS.md`; this matrix | Active plan with five dependency phases, 20 exact prepublication receipt gates, postpublication `BETA-R05`/`BETA-O01`, mapped work packages, non-waivable failures, and a post-V1 horizon. ADR-086 fixed the public-beta floor to Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64 and left the unpublished `0.1.0-beta.1` four-platform draft unused. High-level A-01 through A-11 scope is otherwise fixed, while final candidate and postpublication evidence remain open |
