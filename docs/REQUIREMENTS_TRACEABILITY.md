...
| Native OTA transaction | `updater.py`, Core admin routes, dashboard Updates page | N-1 valid/current, partial/oversize/checksum/HTTP/redirect/cancel, disk/lock/crash, backup, health/rollback/restart/concurrency tests; real Windows adapter capability test | Verification/staging implemented; all real platforms manual-only until an independent journaled binary+DB rollback helper is implemented and packaged-smoked |
