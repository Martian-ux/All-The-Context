...
|---|---|---|
| UPDATER-04 — frozen Windows Core must not start across unsafe persisted recovery state | `windows_update_helper.py::ensure_recovery_before_core`; plain-file and bounded-state validation; operation-owned staged manifest/artifact evidence; atomic `updates/startup-recovery.json`; packaged `--recovery-doctor`; `test_core_start_guard_blocks_unreadable_state_and_records_safe_diagnostic`; `test_core_start_guard_blocks_invalid_journal_and_keeps_core_down`; `test_core_start_guard_resets_pre_cutover_install_without_transaction`; `test_core_start_guard_does_not_reset_pre_cutover_install_without_valid_operation`; `test_core_start_guard_does_not_reset_valid_format_unknown_operation`; `test_core_start_guard_rejects_invalid_handoff_identity`; `test_core_start_guard_does_not_treat_pending_identity_as_unbound`; ADR-183 | Implemented and locally tested. Unreadable or invalid state/journal blocks Core before vault startup, leaves the original state/database untouched, and records only fixed allowlisted recovery facts. A valid pre-cutover `installing` state with a valid operation ID, matching staged manifest/artifact, and no transaction/directory/identity evidence resets to an error state without touching the vault; missing/invalid operation, staging, or active-handoff evidence still blocks. The health-check environment is scheduler suppression only and cannot bypass this guard. Only the packaged doctor's `startup_recovery` field is content-free; its existing operator path fields remain. Exact packaged and hosted evidence remain open |
