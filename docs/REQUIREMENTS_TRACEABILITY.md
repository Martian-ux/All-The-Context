...
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075/ADR-086/ADR-088/ADR-089/ADR-092; `acceptance_receipt.py`; both receipt-bundle templates; `STATUS.md`; this matrix | ADR-092 keeps the 20-gate `certification_v1` contract intact and adds an exact six-gate `lean_public_beta_v1` initial-release profile: Windows/Ubuntu exact first-run (`BETA-L01`/`BETA-L02`), security (`BETA-S06`), exact source (`BETA-R01`), human key custody (`BETA-R02`), and immutable inventory (`BETA-R03`). Lean approval requires four explicit human acknowledgements and does not convert the incomplete certification ledger into pass/skip/waive credit. `BETA-R05`/`BETA-O01` stay postpublication. ADR-086 fixes support to Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64; macOS remains unsupported. The live unpublished beta.3 identity remains draft `371617909` / source `89f3973` / RC `32010253144` / candidate `804afcd9…1d77f369` (31 assets), unsigned and unpublished |
...
| Signed community updates | Ed25519 manifests/keyring with active beta key `release-2026-b` and revoked prepublication predecessor `release-2026-a`, installed/frozen package identity, trust-backed available-channel diagnostics, explicit `unpublished` state for only the empty canonical beta channel, owner-admin immutability preflight without an Actions admin token, authenticated paginated draft collision detection and numeric release-ID controls through publication (ADR-068), canonical packaged beta endpoint, pinned GitHub release-asset redirect, checksums, SBOM/provenance, Windows recovery helper, bounded non-sensitive rollback failure state, and one exact-state persisted rollback-smoke re-entry (ADR-082); `accept_exact_candidate` reopens a verified same-version offer for transactional acceptance smoke without network I/O; `publication_gate.py` + receipt recompute (ADR-059) refuse forged/incomplete/mixed digests | Source machinery for same-version beta exact-candidate OTA acceptance/rollback is honest and fail-closed. Unpublished immutable-mode drafts are uniquely bound by numeric ID, target, state, and exact asset names/digests/sizes; any matching draft closes the version slot even while tag routes return 404. The Windows packaged smoke may re-enter only `rolling_back` / `rollback_retry_required` once, then must still prove terminal rollback, restored binaries/database, prior-Core health, uninstall, and cleanup; every other state or repeated failure blocks. The unpublished four-platform `beta.1` draft is ID `367337056` / source `563a397` / RC `31285545048` / candidate `ba17eeec…42842ebe` (55 assets) and remains historical; the earlier `360008392` / `488150…` episode is historical only. The unpublished `beta.2` identity is likewise occupied and is not rebound. The unpublished Windows/Linux `beta.3` draft is ID `371617909` / source `89f3973` / RC `32010253144` / candidate `804afcd9…d77f369`; prepublication key rotation leaves it occupied and unpublished. The operator-controlled primary and one separate encrypted backup now satisfy the beta custody count, and the backup passed a human-entered restore test; a fresh Windows/Linux-only `beta.4` candidate, candidate-bound R02 receipt, deliberate sole-maintainer approval, offline-signed Windows channel bytes, public download smoke (BETA-R05), and exact packaged candidate rollback receipt remain pending. A real first-published-beta-to-successor N-1 transaction remains post-V1 |
...
| Public support and launch-watch sequencing | `SUPPORT.md`, `docs/KNOWN_ISSUES.md`, `SECURITY.md`, `docs/operations/RUNBOOK.md`, README links, release source validation, ADR-075/ADR-092 | Candidate creation fails closed unless support, known issues, security intake, and recovery guidance are present and linked. This source readiness is not an O01 pass. The exact selected-profile maintainer approval precedes publication; `BETA-R05` public smoke and `BETA-O01` live paths/triaged launch watch close only afterward with exact candidate-bound operational evidence. The initial lean bundle has six gates and four acknowledgements; the certification bundle has the unchanged 20 |
