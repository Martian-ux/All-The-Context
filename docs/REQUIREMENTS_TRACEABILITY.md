...
| Wave 3 falsification program | ADR-046; active Wave 3 manifest and governance test; five visible B01/O01/P01/E01b/M2 worktree tasks from one immutable coordinator commit | Active with coordinator-only integration, new-file-only ownership, no external code/models/providers or operator Core, and explicit preservation of failed/unsupported/held/killed/not-exercised cells; no result accepted yet |
