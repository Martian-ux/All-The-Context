...
| ZF-012 — Project Context Capsule compiler | `project_continuity.py`; `GET /v1/admin/projects`, `GET /v1/admin/projects/{project_id}/capsule`; `tests/unit/test_project_continuity.py`, `tests/unit/test_project_runtime.py` | Implemented locally: current/lifecycle-eligible evidence is filtered before selection; items carry authority and provenance; default 12,000-character/32-item budgets report exact omissions and truncation; `optimized_rebuild` equals the full rebuild oracle after restart/reordering; the projection is read-only and Core-derived |
| ZF-013 — project graph in Memory Lab | retained execution-plan requirement and current capsule boundary | Open: no graph store, graph expansion, learned relation family, or graph promotion is implemented or credited. The deterministic capsule baseline is the prerequisite for later graph evaluation |
