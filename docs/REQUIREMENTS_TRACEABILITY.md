...
| Memory Truth review corrections | migrations 010/011; `storage.py`; `export.py`; Memory Truth/storage/export/Core API/relay tests; ADR-107 | Implemented locally: comment-aware restart-safe migration recovery; identity-key synchronization across observation update/correction/restore and imported rows; validated source-rebuild provenance with fail-closed restore tampering; SQL-bounded truth pagination/counting; public-limit bounds for superseded/conflict/evidence arrays; and idempotent manual-approval evidence links. Full-suite, hosted CI, live Core, and private-export inspection remain outside this lane. |
