...
|---|---|---|
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075/ADR-086/ADR-088/ADR-089; `STATUS.md`; this matrix | Active plan with five dependency phases, 20 exact prepublication receipt gates, postpublication `BETA-R05`/`BETA-O01`, mapped work packages, non-waivable failures, and a post-V1 horizon. ADR-086 fixed the public-beta floor to Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64. The live unpublished `0.1.0-beta.1` four-platform identity is draft `367337056` / source `563a397` / RC `31285545048` / candidate `ba17eeec…42842ebe` (55 assets) and is never retargeted, deleted, or published; the earlier `360008392` / `488150…` episode is historical only. The unpublished `0.1.0-beta.2` Windows/Linux-only identity remains an occupied historical draft and is not rebound. Source inspection of `6151e1f` found a BETA-P06 visible-focus cascade defect on the dashboard search field; ADR-089 records the source `:focus-within` wrapper treatment and advances the active source version to `0.1.0-beta.3`. This does not pass P06. Candidate dispatch stays blocked pending this source correction, review, and exact-main green. High-level A-01 through A-11 scope is otherwise fixed |
