...
| One-click desktop packaging | Windows installer, macOS app/DMG, Linux portable archive; version-matched recovery helper/mode; bounded native-tool failure diagnostics and a single 30-second managed-Core startup window; packaged first-run smoke startup-key cleanup (`remove_smoke_windows_startup_key`) | Packaging and recovery surfaces are integrated; the roadmap-baseline native matrix is green on Windows, Ubuntu, macOS ARM, and macOS Intel. Successful Windows packaged smoke now deletes the exact run-owned smoke startup override key after value removal and fails closed on nonempty/unexpected keys; ordinary product Run-key behavior is unchanged. Exact release artifacts and real-machine/downloaded-artifact receipts remain pending |
