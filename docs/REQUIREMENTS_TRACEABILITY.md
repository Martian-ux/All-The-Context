...
| PACKET-A-09 — unified bounded parsing for embedded narrative JSON | `_parse_bounded_json_bytes`, `_validate_narrative`, content-free `_require_keys`/duplicate/non-finite diagnostics, and 28 focused regression tests | The fenced narrative binding uses the same byte/depth/node/string/numeric/duplicate/non-finite/malformed policy as the top-level JSON document. Exact expected keys remain enforced without exposing unknown or secret-like key names; discarded 1,100-level values and rebound narrative/code-owned digests fail closed. The remediation is recorded in the committed Packet A chain through `ec57058e36a111f41ef24e73c6827d59f324e436`; independent reviewer acceptance remains pending |
...
| PACKET-A-11 — independent authority-source and safe-path closure | `EXPECTED_CONTRACT_SOURCE_SHA256`, normalized `bench/packet_a_contract.py` provenance/content/narrative bindings, exact concrete-path checks, exception-graph regressions, and hostile path probes | Contract-source drift fails without coordinated rebinding; virtual/path-like/subclassed inputs fail before filesystem dispatch; JSON, UTF-8, encoding, and missing-file failures expose neither raw content nor chained path/exception objects. No experiment, manifest, result, private data, product/runtime change, or Packet A acceptance claim exists |
