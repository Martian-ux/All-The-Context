...
| Required MCP tools | `mcp_adapter.py`; MCP SDK v2 `MCPServer` and public transport runners; `observed_at` input, automatic disposition/record/reason/time/version output, and explicit reversible `forget_context`; STDIO/Streamable HTTP/OAuth contract tests | Implemented locally; MCP v2 contract, legacy 2025-era handshake, managed STDIO restart, correction, queued-forget, PKCE refresh/revocation, bearer, Core-authority, and bounded-content suites pass. Ordinary MCP remains L0; lifecycle-aware L1-L3 hooks are not claimed |
