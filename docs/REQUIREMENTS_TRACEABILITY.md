...
|---|---|---|
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075; `STATUS.md`; this matrix | Active plan with five dependency phases, 20 exact prepublication receipt gates, postpublication `BETA-R05`/`BETA-O01`, mapped work packages, non-waivable failures, and a post-V1 horizon; high-level A-01 through A-11 scope is fixed, while final candidate and postpublication evidence remain open |
| Cross-platform Core | `config.py`, `lifecycle.py`, `platform_compat.py`; platform/package smoke tests; `macos_acceptance_preflight.py`; `run_macos_native_supporting_checks.py`; ADR-085 | Mandatory floor: Windows 11 x86-64, macOS 26 ARM64/x86-64, Ubuntu 24.04 LTS x86-64 GNOME with working Secret Service/GNOME Keyring; other Linux environments experimental. Roadmap baseline `1d44fdd` passed all nine hosted jobs. Mac host/candidate preparation is deterministic and content-free but explicitly emits no receipt; exact Windows/macOS build/patch, operator-owned clean-machine acceptance, and final release-SHA receipts are pending |
...
| Windows/macOS/Linux CI | `.github/workflows/ci.yml` source, dashboard, native-package matrices, and content-free native Mac preflight | All three OS families are mandatory under A-11. All nine roadmap-baseline jobs were observed green at `1d44fdd` in run 30177362472. Current source adds native architecture/Rosetta/tool/OS preflight to both Mac package jobs; hosted observation on the integrated SHA, exact support targets, final frozen release SHA, and operator-owned clean-machine receipts remain pending |
| One-click desktop packaging | Windows installer, macOS app/DMG, Linux portable archive; version-matched recovery helper/mode; bounded native-tool failure diagnostics and a single 30-second managed-Core startup window; packaged first-run smoke startup-key cleanup (`remove_smoke_windows_startup_key`); ADR-081 static cryptography OpenSSL and ADR-085 native preparation contracts | Packaging and recovery surfaces are integrated; the roadmap-baseline native matrix is green on Windows, Ubuntu, macOS ARM, and macOS Intel. macOS packaging installs force static OpenSSL for source-built cryptography and fail closed if `otool` still reports dynamic `libssl.3.dylib` or `libcrypto.3.dylib`. Direct-package verification now mounts the DMG and binds bundle identity/version, safe links, main/MCP/recovery architecture, structural seal, and the identity-free unsigned boundary to its report. Successful Windows packaged smoke deletes its exact run-owned startup override key. Exact release artifacts and real-machine/downloaded-artifact receipts remain pending |
| Signed community updates | Ed25519 manifests/keyring with active beta key `release-2026-a`, installed/frozen package identity, trust-backed available-channel diagnostics, explicit `unpublished` state for only the empty canonical beta channel, owner-admin immutability preflight without an Actions admin token, authenticated paginated draft collision detection and numeric release-ID controls through publication (ADR-068), canonical packaged beta endpoint, pinned GitHub release-asset redirect, checksums, SBOM/provenance, Windows recovery helper, bounded non-sensitive rollback failure state, and one exact-state persisted rollback-smoke re-entry (ADR-082); `accept_exact_candidate` reopens a verified same-version offer for transactional acceptance smoke without network I/O; `publication_gate.py` + receipt recompute (ADR-059) refuse forged/incomplete/mixed digests | Source machinery for same-version beta1 exact-candidate OTA acceptance/rollback is honest and fail-closed. Unpublished immutable-mode drafts are now uniquely bound by numeric ID, target, state, and exact asset names/digests/sizes; any matching draft closes the version slot even while tag routes return 404. The Windows packaged smoke may re-enter only `rolling_back` / `rollback_retry_required` once, then must still prove terminal rollback, restored binaries/database, prior-Core health, uninstall, and cleanup; every other state or repeated failure blocks. A corrected candidate rebuild, key backups, deliberate sole-maintainer publication, offline-signed channel bytes, public download smoke (BETA-R05), and exact packaged candidate rollback receipt remain pending; real beta1-to-beta2 N-1 remains the first post-V1 beta2 gate |
