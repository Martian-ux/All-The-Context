...
| First usable V1 beta contract | `ROADMAP_TO_V1.md`; ADR-053/ADR-075/ADR-086/ADR-088/ADR-089/ADR-092/ADR-095; `acceptance_receipt.py`; both receipt-bundle templates; `STATUS.md`; this matrix | ADR-092 keeps the 20-gate `certification_v1` contract intact and adds the exact six-gate `lean_public_beta_v1` initial-release profile: Windows/Ubuntu exact first run (`BETA-L01`/`BETA-L02`), security (`BETA-S06`), exact source (`BETA-R01`), human key custody (`BETA-R02`), and immutable inventory (`BETA-R03`). Lean approval requires four explicit human acknowledgements and does not convert incomplete certification into pass/skip/waive credit. `BETA-R05`/`BETA-O01` stay postpublication. ADR-086 fixes support to Windows 11 x86-64 and Ubuntu 24.04 LTS x86-64; macOS remains unsupported. ADR-095 retains occupied beta.5 as contradictory unpublished history and makes beta.6 the active source slot; its post-build identity and receipts belong in the external exact prepublication ledger |
